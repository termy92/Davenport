...
public class JDBMaterialBatch {
			...
			if (JUtility.isStringPatternValid(batchREGEX, getBatch())) {
				if (getMaterialBatchProperties() == false) {
					...
					if (getStatus().equalsIgnoreCase("")) {
						setStatus(ctrl.getKeyValueWithDefault("DEFAULT BATCH STATUS", "Restricted",
					...
					if (getExpiryDate() != null) {
						if (expiry != null) {
							if (expiry.equals(getExpiryDate()) == false) {
								setErrorMessage("Cannot override batch expiry date. Batch [" + getMaterial() + "/"
										+ getBatch() + "] already exists with expiry date of "
	...
	public boolean create() {
		...
		// .getSQL("JDBMaterialBatch.create"));
	...
	public boolean delete() {
		...
		// .getConnection(getSessionID())
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements().getSQL("JDBMaterialBatch.delete"));
		// stmtupdate.setString(1, getMaterial());
		// stmtupdate.setString(2, getBatch());
		// stmtupdate.execute();
		// stmtupdate.clearParameters();
	...
	public Vector<JDBMaterialBatch> getMaterialBatchData(PreparedStatement criteria) {
		...
		// result.addElement(new JDBMaterialBatch("material", "batch", "status",
		// JUtility.getSQLDateTime()));
		// } else {
		try {
			rs = criteria.executeQuery();

			while (rs.next()) {
				result.addElement(new JDBMaterialBatch(rs.getString("material"), rs.getString("batch_number"), rs
						.getString("status"), rs.getTimestamp("expiry_date")));
			}
	...
	public boolean getMaterialBatchProperties() {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBMaterialBatch.getMaterialBatchProperties"));
		// stmt.setFetchSize(1);
		...
		// result = true;
		// } else {
		// setErrorMessage("Unknown Material Batch [" + getMaterial() + " " +
		// getBatch() + "]");
		// }
		// rs.close();
		// stmt.close();
	...
	public boolean isValidMaterialBatch() {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBMaterialBatch.isValidMaterialBatch"));
		// stmt.setString(1, getMaterial());
		// stmt.setString(2, getBatch());
		// stmt.setFetchSize(1);
		// rs = stmt.executeQuery();
	...
	public void setBatch(String batch) {
		dbMaterialBatch = batch.toUpperCase();
	}
	...
	public boolean update() {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBMaterialBatch.update"));
		//
