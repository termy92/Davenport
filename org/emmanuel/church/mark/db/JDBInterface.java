...
public class JDBInterface {
	...
	public boolean create() {
		...
		if (isValid() == true) {
			...
			// .prepareStatement(
			// Common.hostList.getHost(getHostID()).getSqlstatements().getSQL("JDBInterface.create"));
	...
	public boolean delete() {
		...
		// .getConnection(getSessionID())
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements().getSQL("JDBInterface.delete"));
		// stmtupdate.setString(1, getInterfaceType());
		// stmtupdate.setString(2, getInterfaceDirection());
		// stmtupdate.execute();
		// stmtupdate.clearParameters();
	...
	public Vector<JDBInterface> getInterfaceData(PreparedStatement criteria) {
		...
		// result.addElement(new JDBInterface("interfaceType",
		// "interfaceDirection", "Path", "Enabled", "Device"));
		// } else {
		try {
			rs = criteria.executeQuery();

			while (rs.next()) {
				result.addElement(new JDBInterface(rs.getString("interface_Type"), rs.getString("interface_Direction"),
						rs.getString("Path"), rs.getString("Enabled"), rs.getString("Device")));
			}
			rs.close();
	...
	public LinkedList<JDBInterface> getInterfaceList(String device, String interfaceDirection) {
		...
		// String schemaName =
		// Common.hostList.getHost(getHostID()).getDatabaseParameters().getjdbcDatabaseSchema();
		String ad = "asc";

		JDBQuery query = new JDBQuery(getHostID(), getSessionID());
		query.clear();

		// query.addText(JUtility.substSchemaName(schemaName,
		// "select * from {schema}sys_interface"));
		...
		try {
			...
			while (rs.next()) {
				JDBInterface temp = new JDBInterface(rs.getString("interface_Type"), interfaceDirection,
						rs.getString("Path"), rs.getString("Enabled"), rs.getString("Device"));
				result.add(temp);
			}
			rs.close();
	...
	public boolean getInterfaceProperties() {
		...
		// .getSQL("JDBInterface.getInterfaceProperties"));
		// stmt.setFetchSize(1);
	...
	public boolean isValidInterface() {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBInterface.isValidInterface"));
		// stmt.setString(1, getInterfaceType());
		// stmt.setString(2, getInterfaceDirection());
		// stmt.setFetchSize(1);
		// rs = stmt.executeQuery();
	...
	public boolean update() {
		...
		// .getConnection(getSessionID())
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements().getSQL("JDBInterface.update"));
		//
