...
public class JDBMaterial {
	...
	public JDBMaterial(String material, String description, String type, String baseUom, Integer shelflife,
			String shelflifeuom, String shelfliferule, String equipmentType) {
	...
	public Date getRoundedExpiryDate(Date inputDate) {
		...
		if (BatchExpiryTimeRoudingMode.equals("")) {
			BatchExpiryTimeRoudingMode = ctrl.getKeyValueWithDefault("BATCH EXPIRY TIME", "-",
					"Round Batch Expiry Time");
		}
	...
	public boolean create() {
		...
		if (isValid() == true) {
			...
			if (getMaterialProperties(getMaterial()) == true) {
				...
				// stmtupdate =
				// Common.hostList.getHost(getHostID()).getConnection(getSessionID()).prepareStatement(Common.hostList.getHost(getHostID()).getSqlstatements().getSQL("JDBMaterial.create"));
	...
	public boolean delete() {
		...
		// result = true;
		// } else {
		// setErrorMessage(noofuoms.toString() +
		// " uom conversions exist for Material " + getMaterial());
		// }
		// } else {
		// setErrorMessage(noofbatches.toString() +
		// " batches exist for Material " + getMaterial());
		// }
		// }
		// } catch (SQLException e) {
		// setErrorMessage(e.getMessage());
	...
	public Integer getMaterialBatchCount() {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBMaterial.getMaterialBatchCount"));
		// stmt.setFetchSize(1);
	...
	public LinkedList<JDBMaterialBatch> getMaterialBatches() {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBMaterial.getMaterialBatches"));
		// stmt.setFetchSize(1);
		// stmt.setString(1, getMaterial());
		// rs = stmt.executeQuery();
		//
		// while (rs.next()) {
		// JDBMaterialBatch mb = new JDBMaterialBatch(rs.getString("material"),
		// rs.getString("batch"),
		// rs.getTimestamp("expiry_date"), rs.getString("status"));
	...
	public Vector<JDBMaterial> getMaterialData(PreparedStatement criteria) {
		...
		// result.addElement(new JDBMaterial("material", "description", "type",
		// "base_uom", 0, "shelf life uom",
		// "rounding rule", "equipmentType"));
		// } else {
		try {
			rs = criteria.executeQuery();

			while (rs.next()) {
				result.addElement(new JDBMaterial(rs.getString("material"), rs.getString("description"), rs
						.getString("material_type"), rs.getString("base_uom"), rs.getInt("shelf_life"), rs
						.getString("shelf_life_uom"), rs.getString("shelf_life_rule"), rs.getString("equipment_type")));
			}
			rs.close();
	...
	public boolean getMaterialProperties(String lmaterial) {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBMaterial.getMaterialProperties"));
	...
	public Integer getMaterialUomCount() {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBMaterial.getMaterialUomCount"));
		// stmt.setFetchSize(1);
		// stmt.setString(1, getMaterial());
		// rs = stmt.executeQuery();
		//
	...
	public LinkedList<JDBMaterialUom> getMaterialUoms() {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBMaterial.getMaterialUoms"));
		// stmt.setFetchSize(1);
		// stmt.setString(1, getMaterial());
		// rs = stmt.executeQuery();
		//
		// while (rs.next()) {
		// JDBMaterialUom mu = new JDBMaterialUom(getHostID(), getSessionID(),
		// rs.getString("material"),
		// rs.getString("uom"), rs.getString("ean"), rs.getString("variant"),
		// rs.getInt("numerator"),
		// rs.getInt("denominator"));
		// uomList.addLast(mu);
		// }
		// rs.close();
		// stmt.close();
	...
	public boolean isValidMaterial() {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBMaterial.isValidMaterial"));
		// stmt.setString(1, getMaterial());
		// stmt.setFetchSize(1);
		// rs = stmt.executeQuery();
		//
	...
	public boolean update() {
		...
		// .getConnection(getSessionID())
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements().getSQL("JDBMaterial.update"));
		//
		...
		// stmtupdate.setString(14, getMaterial());
		//
		// stmtupdate.execute();
		// stmtupdate.clearParameters();
		// Common.hostList.getHost(getHostID()).getConnection(getSessionID()).commit();
