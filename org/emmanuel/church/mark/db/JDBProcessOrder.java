...
public class JDBProcessOrder {
	...
	private JDBProcessOrder(String process_order, String material, String description, String status, String location,
			Timestamp due_date, String recipe, String palletStatus, String reqdResource) {
		clear();
	...
	public boolean create() {
		...
		if (isValid() == true) {
			...
			if (getProcessOrderProperties(getProcessOrder()) == true) {
	...
	public Vector<JDBProcessOrder> getProcessOrderData(PreparedStatement criteria) {
		...
		// result.addElement(new JDBProcessOrder("process_order", "material",
		// "description", "status", "location_id",
		// null, "recipe_id", "pallet_status", "resource"));
		// } else {
		try {
			rs = criteria.executeQuery();

			while (rs.next()) {
				result.addElement(new JDBProcessOrder(rs.getString("process_order"), rs.getString("material"), rs
						.getString("description"), rs.getString("status"), rs.getString("location_id"), rs
						.getTimestamp("due_date"), rs.getString("recipe_id"), rs.getString("default_pallet_status"), rs
						.getString("required_resource")));
			}
			rs.close();
	...
	public boolean getProcessOrderProperties(String process_order) {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBProcessOrder.getProcessOrderProperties"));
	...
	public boolean isValidProcessOrder() {
		...
		// .prepareStatement(
		// Common.hostList.getHost(getHostID()).getSqlstatements()
		// .getSQL("JDBProcessOrder.isValidProcessOrder"));
	...
	public boolean update() {
		...
		if (isValid() == true) {
			...
			// stmtupdate.setString(13, getProcessOrder());
			//
			// stmtupdate.execute();
			// stmtupdate.clearParameters();
			// Common.hostList.getHost(getHostID()).getConnection(getSessionID()).commit();
