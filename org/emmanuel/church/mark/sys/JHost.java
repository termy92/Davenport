...
import org.emmanuel.church.mark.db.JDBSQLStatement;
import org.emmanuel.church.mark.db.JDatabaseParameters;
...
public class JHost {
	...
	private final Hashtable<String, Connection> connections = new Hashtable<String, Connection>();
	private final Hashtable<String, Boolean> connected2 = new Hashtable<String, Boolean>();
		...
		if (connections.containsKey(sessionID) == false) {
			try {
				...
				try {
					...
					logger.fatal(ex);
					// if (Common.sd.getData(sessionID,
					// "silentExceptions").equals("Yes") == false) {
					// JUtility.errorBeep();
					// JOptionPane.showMessageDialog(null, WordUtils.wrap(err,
					// 100), "Database Connection Error ("
					// + getSiteDescription() + ")", JOptionPane.ERROR_MESSAGE);
					// }
					result = false;
				}
			} catch (Exception ex) {
				...
				// "silentExceptions").equals("Yes") == false) {
				// JUtility.errorBeep();
				// JOptionPane.showMessageDialog(null, "Invalid jdbc driver [" +
				// ex.getMessage() + "]",
				// "Login Error (" + getSiteDescription() + ")",
