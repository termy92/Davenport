...

{
	...
	public JInternalFrameMenuTree(String menuID, String title, boolean resizable, boolean closable,
			boolean maximizable, boolean iconifiable, JMenuBar menubar, JMenu menu) {
		...
		setLocation((screenSize.width - (screenWidth + Common.LFTreeMenuAdjustWidth)) / 2,
				(screenSize.height - (screenHeight + Common.LFTreeMenuAdjustHeight)) / 2);
		setResizable(true);
	...
	private void buildMenuTree(String menuID, DefaultMutableTreeNode node, JMenuBar menubar, int level, JMenu menu) {
		...
		try {
			...
			// .getHost(Common.selectedHostID)
			// .getConnection(Common.sessionID)
			// .prepareStatement(
			// Common.hostList.getHost(Common.selectedHostID).getSqlstatements()
			// .getSQL("JInternalFrameMenuTree.buildMenuTree"));
			...
			// while (rs.next()) {
			// JMenuOption menuOption = new JMenuOption(Common.selectedHostID,
			// Common.sessionID);
			// menuOption.load(rs);
			// TODO: fill out menu options

			CustomMenus menus = new CustomMenus();
			HashMap<JMenuOption, HashMap<String, JMenuOption>> maps = menus.getMenuMap();
			
			for(Map.Entry<JMenuOption, HashMap<String, JMenuOption>> entry : maps.entrySet()){
				JMenuOption gMenu = entry.getKey();
				DefaultMutableTreeNode newnode = new DefaultMutableTreeNode(gMenu);
				node.add(newnode);
				
				if (gMenu.moduleType.equals("MENU")) {
					JMenu newmenu = new JMenu(gMenu.description);
					newmenu.setFont(Common.font_menu);
					newmenu.setMnemonic(gMenu.mnemonic);
					...
					buildMenuTree(gMenu.moduleID, newnode, menubar, level + 1, newmenu);
				}
				
				HashMap<String, JMenuOption> map = entry.getValue();
				if(map != null){
					for(Map.Entry<String, JMenuOption> e : map.entrySet()){
						String cMenu = e.getKey();
						JMenuItem newmenuitem = new JMenuPulldownMenuItem(e.getValue());
						m.setModuleId(e.getValue().moduleID);
...
//						DefaultMutableTreeNode cNode = e.getValue();
//						node.add(cNode);
					}
				}
			}
...
//			JMenuOption menuOption = new JMenuOption(Common.selectedHostID, Common.sessionID);
//			menuOption.menuID = "PLAYERS";
//			menuOption.moduleID = "PLAYERSS";
//			menuOption.description = "Players";
//			menuOption.moduleType = "SUBMENU";
//			menuOption.sequenceID = 0;
//
