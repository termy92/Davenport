	...
	public static LinkedList<JDBSQLStatement> loadSQLStatements(String host, String driver, String filename) {
		...
		try {
			...
			do {
				...
				try {
					statementId = xmltest.findXPath("//SQL/jdbcDriver[@Type='" + driver + "']/statement[" + sNumber
							+ "]/id");
					statementText = xmltest.findXPath("//SQL/jdbcDriver[@Type='" + driver + "']/statement[" + sNumber
							+ "]/text");
					// statementText = statementText.replace("{schema}",
					// Common.hostList.getHost(host)
					// .getDatabaseParameters().getjdbcDatabaseSchema());
					// statementText = statementText.replace("{database}",
					// Common.hostList.getHost(host)
