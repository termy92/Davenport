...
import org.emmanuel.church.mark.db.JDBDDL;
...
public class JXMLSchema {
	...
	public static int getSchemaVersion() {
		 ...
		 * int result = -1;
		 * 
		 * String version = ""; JXMLDocument xmltest = new
		 * JXMLDocument("xml/schema/schema." +
		 * Common.hostList.getHost(Common.selectedHostID
		 * ).getDatabaseParameters().getjdbcDriver() + ".xml");
		 * 
		 * version = JUtility.replaceNullStringwithBlank(xmltest
		 * .findXPath("//DDL/currentVersion"));
		 * 
		 * if (version.equalsIgnoreCase("") == false) { result =
	...
	public static LinkedList<JDBDDL> loadDDLStatements(String driver, String directory) {
		...
		// for (int ver =
		// Common.hostList.getHost(Common.selectedHostID).getSchemaVersion() +
		// 1; ver <= JVersion
		// .getSchemaVersion(); ver++) {
		//
		...
		// Common.hostList.getHost(Common.selectedHostID)
		// .getDatabaseParameters().getjdbcDatabaseSchema());
		// Text = Text.replace("{database}",
		// Common.hostList.getHost(Common.selectedHostID)
		// .getDatabaseParameters().getjdbcDatabase());
